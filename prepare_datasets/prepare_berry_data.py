...
#!/bin/bash
"""Generate labeled and unlabeled data for berry train.

Example:
python3 object_detection/prepare_berry_data.py
"""
...
assert 'BERRYDIR' in os.environ, 'Set BERRYDIR in bash'
DATA_DIR = os.environ['BERRYDIR']


def prepare_berry_data(seed=1, percent=50.0, version=2017):
  """Prepare berry data for Semi-supervised learning

  Args:
    seed: random seed for data split
    percent: percentage of labeled data
    version: BERRY data version
  """
  def _save_anno(name, images, annotations):
    ...
    path = '{}/{}'.format(BERRYANNODIR, 'semi_supervised')
    if not os.path.exists(path):
      os.mkdir(path)

    with open(
        '{root}/{folder}/{save_name}.json'.format(
            save_name=name, root=BERRYANNODIR, folder='semi_supervised'),
        'w') as f:
  ...
  np.random.seed(seed)
  BERRYANNODIR = os.path.join(DATA_DIR, 'annotations')

  anno = json.load(open(os.path.join(BERRYANNODIR,
...
if __name__ == '__main__':
  ...
  prepare_berry_data(args.seed, args.percent, args.version)
